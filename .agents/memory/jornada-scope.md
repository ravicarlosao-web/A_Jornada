...
**Update 4:** Added rival/position-battle results per season, contract clauses (release/goal-bonus/signing), national-team call-ups and titles, international club transfers (fama/overall gated), and sponsorship deals — all surfaced as season summary/manchete entries rather than new interactive screens. Hall da Fama moved from localStorage-only to also persisting via a Postgres table + api-server routes, with localStorage kept as an offline fallback and merged into a combined ranking on read. Any state shape added to `Jogador`/`Contrato` must include a backfill/default in the localStorage load path in `useCareer.ts`, since existing saves won't have new fields.

**Update 5:** Sponsorship and contract negotiation became interactive: a dedicated PatrocinioScreen (accept/decline) before the sponsorship is added to `jogador.patrocinios`, and a "Negociar" action per contract offer that can improve terms or make the club walk away. List items shown across multiple interactions (e.g. contract offers that can be removed) must be keyed and tracked by a stable generated `id`, never by array index — index-based tracking breaks as soon as an item is removed/reordered. Also always guard negotiation-style mechanics so the player can't negotiate away the last remaining option and get stuck.
